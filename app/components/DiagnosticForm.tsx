...
function toInput(f: FormData) {
  return {
    ...
    type_prise_terre:         (f.type_prise_terre as 'BOUCLE_FOUILLE' | 'PIQUETS' | 'INCONNU') || undefined,
    ...
    type_diff_borne:          (f.type_diff_borne as 'A' | 'B' | 'AC' | 'INCONNU') || undefined,
...
function NumberStepper({
  value, onChange, step = 1, values, min = 0, max, unit, placeholder,
}: StepperProps) {
  ...
  return (
    <div className={`flex overflow-hidden rounded-lg border shadow-sm ${dividerCls}`}>
      {/* Bouton − */}
...
const LEVEL_CLS = {
  BLOQUANT:      'bg-red-100 text-red-700 border border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800',
  MAJEUR:        'bg-orange-100 text-orange-700 border border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800',
  AVERTISSEMENT: 'bg-yellow-100 text-yellow-700 border border-yellow-200 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-800',
};
const RISK_CLS: Record<string, string> = {
  ...
  'ELEVÉ':  'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300',
...
function LevelBadge({ level }: { level: keyof typeof LEVEL_CLS }) {
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold tracking-wide ${LEVEL_CLS[level]}`}>
      {level}
    </span>
  );
}
...
const Icons = {
  ...
  ground: () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
      ...
      <path d="M12 2v14" /><path d="M5 16h14" /><path d="M8 20h8" /><path d="M11 22h2" />
    </svg>
  ),
  ...
  circuit: () => <Icon d="M18 20V10M12 20V4M6 20v-6M2 10l4-4 4 4 4-4 4 4M2 14h20" />,
  car: () => <Icon d="M5 17H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h14l4 4v6a2 2 0 0 1-2 2h-2M5 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0M15 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0" />,
  alert: () => <Icon d="M12 9v4M12 17h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />,
  check: () => (
...
function BoolSelect({ value, onChange }: { value: BoolStr; onChange: (v: BoolStr) => void }) {
  return (
    <select className={SELECT} value={value} onChange={e => onChange(e.target.value as BoolStr)}>
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        <span className="font-medium text-slate-600 dark:text-slate-400">Score conformité</span>
        <span className="font-bold tabular-nums text-slate-900 dark:text-slate-100">{score} / 100</span>
      </div>
...
export function DiagnosticForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setTimeout(() => document.getElementById('rgie-results')?.scrollIntoView({ behavior: 'smooth' }), 80);
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <Section icon={<Icons.ground />} title="Mise à la terre">
          <Field label="Résistance de terre">
            ...
            <NumberStepper
              ...
              step={1} min={0} unit="Ω" placeholder="—" />
          </Field>
          <Field label="Type de prise de terre">
            <select className={SELECT} value={form.type_prise_terre}
        ...
        <Section icon={<Icons.diff />} title="Protection différentielle">
          <Field label="Différentiel général présent ?">
            <BoolSelect value={form.diff_general_present}
          ...
          <Field label="Calibre diff. général">
            ...
            <NumberStepper
              ...
              values={DIFF_MA} unit="mA" placeholder="—" />
          </Field>
          <Field label="Nombre de diff. 30 mA">
            ...
            <NumberStepper
              ...
              step={1} min={0} max={15} placeholder="—" />
          </Field>
        </Section>
        ...
        <Section icon={<Icons.shower />} title="Salle de bains">
          <Field label="Salle de bains présente ?">
            <BoolSelect value={form.sdb_presente} onChange={v => set('sdb_presente', v)} />
          </Field>
          <Field label="SDB protégée par diff. 30 mA ?">
            <BoolSelect value={form.sdb_protegee_30mA} onChange={v => set('sdb_protegee_30mA', v)} />
          </Field>
          <Field label="Équipotentialité SDB conforme ?">
            <BoolSelect value={form.sdb_equipotentielle_ok} onChange={v => set('sdb_equipotentielle_ok', v)} />
          </Field>
        </Section>
        ...
        <Section icon={<Icons.circuit />} title="Circuits & câblage">
          <Field label="Nb max de prises par circuit">
            ...
            <NumberStepper
              ...
              step={1} min={0} max={30} placeholder="—" />
          </Field>
          <Field label="Section câble minimale">
            ...
            <NumberStepper
              ...
              values={CABLE_SECTIONS} unit="mm²" placeholder="—" />
          </Field>
          <Field label="Calibre disjoncteur max">
            ...
            <NumberStepper
              ...
              values={BREAKER_AMPS} unit="A" placeholder="—" />
          </Field>
        </Section>
        ...
        <Section icon={<Icons.car />} title="Borne de recharge VE">
          <Field label="Borne VE présente ?">
            <BoolSelect value={form.borne_ve_presente} onChange={v => set('borne_ve_presente', v)} />
          </Field>
          <Field label="Type de différentiel borne VE">
            <select className={SELECT} value={form.type_diff_borne}
        ...
        <button type="submit" disabled={loading}
          ...
          {loading
            ? <span className="flex items-center justify-center gap-2"><SpinnerIcon /> Analyse RGIE en cours…</span>
  ...
  return (
    <div id="rgie-results" className="space-y-4 pt-2">
      ...
      <div className={`overflow-hidden rounded-xl border shadow-sm
        ...
        <div className="grid grid-cols-1 divide-y divide-slate-200 sm:grid-cols-3 sm:divide-x sm:divide-y-0 dark:divide-slate-700">
          <div className="flex flex-col items-center justify-center gap-2 p-5">
            ...
            <span className={`text-lg font-bold ${ok ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-5">
            ...
            <span className={`rounded-full px-4 py-1.5 text-sm font-bold ${RISK_CLS[data.risque] ?? 'bg-slate-100 text-slate-700'}`}>
              {data.risque}
            </span>
          </div>
        </div>
      ...
      {data.non_conformites.length > 0 && (
        <ResultSection title="Non-conformités détectées" count={data.non_conformites.length}>
          <ul className="divide-y divide-slate-100 dark:divide-slate-700">
            {data.non_conformites.map(nc => (
              <li key={nc.code} className="px-5 py-4">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5"><LevelBadge level={nc.niveau} /></div>
                  <div className="min-w-0 flex-1">
                    <p className="mb-0.5 font-mono text-[11px] font-bold text-slate-400 dark:text-slate-500">{nc.code}</p>
                    <p className="text-sm text-slate-800 dark:text-slate-200">{nc.message}</p>
                    {nc.correction && (
                      <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
                        <span className="font-semibold text-blue-600 dark:text-blue-400">Correction : </span>
                        {nc.correction}
                      </p>
                    )}
                  </div>
      ...
      {data.top_causes_probables.length > 0 && (
        <ResultSection title="Causes probables identifiées" count={data.top_causes_probables.length}>
          <ul className="divide-y divide-slate-100 dark:divide-slate-700">
            {data.top_causes_probables.map(c => (
              <li key={c.hypothese_id} className="px-5 py-4">
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div>
                    <p className="font-mono text-[11px] font-bold text-slate-400 dark:text-slate-500">{c.hypothese_id}</p>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{c.resume}</p>
                  </div>
                  <span className="shrink-0 rounded-full bg-blue-100 px-2.5 py-1 text-sm font-bold text-blue-700
      ...
      {data.questions_suivantes.length > 0 && (
        <ResultSection title="Questions pour approfondir" count={data.questions_suivantes.length}>
          <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
            {data.questions_suivantes.map(q => (
              <div key={q.id}
                ...
                <p className="mb-1 font-mono text-[10px] font-bold text-slate-400 dark:text-slate-500">{q.id}</p>
                <p className="text-sm text-slate-700 dark:text-slate-300">{q.texte}</p>
                {q.choix && (
      ...
      {data.plan_correction.length > 0 && (
        <ResultSection title="Plan de correction" count={data.plan_correction.length}>
          <ul className="divide-y divide-slate-100 p-5 dark:divide-slate-700">
            {data.plan_correction.map(a => (
              <li key={a.priorite} className="flex items-start gap-4 py-3.5 first:pt-0 last:pb-0">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-slate-800 dark:text-slate-200">{a.action}</p>
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    {a.refs?.map(r => (
                      <span key={r} className="font-mono text-[10px] text-slate-400 dark:text-slate-500">{r}</span>
                    ))}
                  </div>
                </div>
              </li>
